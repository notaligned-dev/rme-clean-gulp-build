...
1. `sourceFolder` — папка для исходных файлов
2. `buildFolder` — папка для выходных файлов
3. `isProxy` — установите значение `true`, если вы работаете со своим локальным сервером и хотите использовать на нём быструю перезагрузку браузера
4. `localWebsiteURL` — работает только с `isProxy`: `true`. Впишите URL своего локального вебсайта
